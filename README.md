# Carnet de Voyage - JavaFX

[![Static Badge](https://img.shields.io/badge/Télécom-Projet_scolaire-purple)](https://telecomnancy.univ-lorraine.fr)
[![Static Badge](https://img.shields.io/badge/Langage-Java-blue?logo=data:image/svg+xml;base64,)](https://www.java.com/fr/)
[![Static Badge](https://img.shields.io/badge/Framework-JavaFX-blue?logo=JavaFX)](https://openjfx.io)

## Sommaire
1. Sommaire
2. Présentation
3. Installation
4. Organisation du dépôt et informations complémentaires

## Présentation
Projet développé dans le cadre de ma scolarité à [Télécom Nancy](https://telecomnancy.univ-lorraine.fr) (2ème année - 1er semestre)
Extrait du sujet:

> On s’intéresse au développement d’une application qui permet de gérer un carnet de voyage.    

La présente application est constituée d'une interface implémentée en JavaFX avec FXML

## Installation

## Installation
##### Téléchargement
###### Par SSH
```
git@github.com:cos-imo/VoyageFXML.git
```
###### Par HTTPS
```
https://github.com/cos-imo/VoyageFXML.git
```

##### Compilation
Depuis la racine du projet (dossier `VoyageFXML`), utilisez gradlew pour installer les dépendances requises:
```
./gradlew build
```


##### Lancement
Depuis la racine du projet (dossier `VoyageFXML`) exécuter gradlew:
```
./gradlew run
```

## Organisation du dépôt et informations complémentaires
